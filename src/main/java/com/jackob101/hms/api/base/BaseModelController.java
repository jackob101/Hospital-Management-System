...
public abstract class BaseModelController<T extends IEntity> extends BaseController<T> {
    ...
    public ResponseEntity<Object> updateEntity(@RequestBody @Validated(OnUpdate.class) T entity) {

        log.info("Updating " + entityName + " with id: " + entity.getId());

        T saved = service.update(entity);
    ...
    public ResponseEntity<Object> createEntity(@RequestBody @Validated(OnCreate.class) T form, BindingResult bindingResult) throws URISyntaxException {
        log.info("Creating new " + entityName);

        ApiUtils.checkBindings(bindingResult, entityName);

        T saved = service.create(form);
